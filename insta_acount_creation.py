...
def create_instagram_account():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Navigate to Instagram sign-up page
            page.goto("https://www.instagram.com/accounts/emailsignup/")
    
            # Fill in sign-up form
            page.get_by_label('Mobile Number or Email').type(data['email'])
            page.get_by_label('Password').type('G7p#kV2!eR')
            names = data['email'].split('@')[0]
            page.get_by_label('Full Name').type(names)
            page.get_by_label('Username').type(names)
            ...
            page.get_by_role("textbox", name="Confirmation Code").type(Confirmation_Code)
